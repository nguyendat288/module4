...
public class ProductController {
    ...
    public String editPost(@RequestParam MultipartFile uppImg, @RequestParam int index, @ModelAttribute("p") Product product, @RequestParam("catergory.id") int catergoryId) {
        ...
        try {
            FileCopyUtils.copy(uppImg.getBytes(), new File("C:\\Java\\untitled\\src\\main\\webapp\\file\\" + nameImg));
    ...
    public String uppFile(@RequestParam MultipartFile uppImg) {
        ...
        try {
            FileCopyUtils.copy(uppImg.getBytes(), new File("C:\\Java\\untitled\\src\\main\\webapp\\file\\" + fileName));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return "redirect:/products";
